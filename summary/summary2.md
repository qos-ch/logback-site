...
a substitute for complete conversion-word tables and exhaustive property
lists.

**Omitted chapters.** This summary does **not** cover *Receivers*,
*Logging Separation*, *JMX Configuration*, or *Groovy Configuration*.
...
This summary follows the core arc of the Logback manual: introduction;
logger hierarchy and levels; configuration search order and XML; the
appender catalogue; encoders versus layouts; pattern conversion; filters
and turbo filters; MDC; SSL; Joran; and migration from log4j. Receivers,
logging separation, JMX, and Groovy configuration were left out by design.

For property-level detail and canonical samples, use the HTML chapters
under `src/site/pages/manual/`. When upgrading, also read the project
news page for behavioral changes such as mandatory `SimpleSocketServer`
IP whitelisting, caller-data contradiction analysis, and the shift from
`ConsoleAppender.withJansi` to `JansiConsoleAppender`.
